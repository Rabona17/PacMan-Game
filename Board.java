...
/**
* @author      Yuan Gao y1gao@ucsd.edu
*/
public class Board{
  ...
  * Constructor
  *
  * <p> Description: Initialize a board with input size
  *
  * @param: size
  *
  * @return: nothing
  */
  ...
  public void refreshGrid() {
    ...
    for (PacCharacter ghost : ghosts) {
      if (pacman.getRow() == ghost.getRow() && pacman.getCol()
          == ghost.getCol()) {
  ...
  public boolean canMove(Direction direction) {
    ...
    return pacmanRow >= 0 && pacmanRow < GRID_SIZE
    && pacmanCol >= 0 && pacmanCol < GRID_SIZE;
  }
  ...
  /*
  * Depp copy a board Obj
  *
  * <p> Description: deep copy a board
  *
  * @param: null
  *
  * @return: a copyed independent board
  */
  public Board boardClone() {
    Board clonedBoard = new Board(this.GRID_SIZE);
    char[][] clonedGrid = new char[this.GRID_SIZE][this.GRID_SIZE];
    for(int i=0; i<this.grid.length; i++)
    for(int j=0; j<this.grid[i].length; j++)
    clonedGrid[i][j]=this.grid[i][j];
    clonedBoard.grid = clonedGrid;
    boolean[][] visitedGrid = new boolean[this.GRID_SIZE][this.GRID_SIZE];
    for(int i=0; i<this.grid.length; i++)
    for(int j=0; j<this.grid[i].length; j++)
    visitedGrid[i][j]=this.visited[i][j];
    clonedBoard.visited = visitedGrid;
    clonedBoard.score = this.score;
    clonedBoard.pacman = this.pacman.pacClone();
    PacCharacter[] clonedGhost = new PacCharacter[4];
    for(int j=0; j<this.ghosts.length; j++) {
      clonedGhost[j]=this.ghosts[j].pacClone();
    }
    clonedBoard.ghosts = clonedGhost;
    return clonedBoard;
  }
  ...
  public void setGhosts(PacCharacter[] ghosts) {
    this.ghosts = ghosts;
  }
